    ...
    <tbody>
        <c:forEach var="book" items="${bookList}">
            <tr>
                ...
                <td class="text-truncate" style="max-width: 150px;" title="${book.bookName}">${book.bookName}</td>
                <td class="text-truncate" style="max-width: 150px;" title="${book.authorID.authorName}">${book.authorID.authorName}</td>
                <td class="text-truncate" style="max-width: 150px;" title="${book.publisherID.publisherName}">${book.publisherID.publisherName}</td>
                <td class="text-truncate" style="max-width: 50px;" title="${book.categoryID.categoryName}">${book.categoryID.categoryName}</td>
                <td>${book.publishingYear}</td>
                <td>${book.bought}</td>
                <td>${book.quantity}</td>
                <td><fmt:formatNumber value="${book.price}" pattern="$ #" /></td>
                <td><fmt:formatNumber value="${book.discount}" type="percent" /></td>
                <td class="text-truncate" style="max-width: 150px;" title="${book.bookSummary}">${book.bookSummary}</td>
                <td>
